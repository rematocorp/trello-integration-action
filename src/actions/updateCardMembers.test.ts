...
it('adds PR author and assignees to the card and removes unrelated members', async () => {
	...
	await updateCardMembers({ ...conf, trelloOrganizationName: 'remato' }, ['card'])
...
it('ignores incorrectly configured usernames mapping', async () => {
	await updateCardMembers({ ...conf, githubUsersToTrelloUsers: 'phil' }, ['card'])
...
it('throws error when fetching member info fails for unknown reason', async () => {
	...
	await expect(updateCardMembers(conf, ['card'])).rejects.toMatchObject({ response: { status: 500 } })
...
it('skips removing unrelated members when turned off', async () => {
	...
	await updateCardMembers({ ...conf, trelloRemoveUnrelatedMembers: false }, ['card'])
...
it('skips adding when member is already assigned to the card', async () => {
	...
	await expect(updateCardMembers(conf, ['card'])).resolves.not.toThrow()
})
...
it('skips adding when turned off', async () => {
	...
	await updateCardMembers({ ...conf, trelloAddMembersToCards: false }, ['card'])
...
describe('switching card members with reviewers when PR is in review', () => {
	beforeEach(() => {
		...
		getPullRequestMock.mockResolvedValue({ ...prResponse, draft: false })
	...
	it('skips when PR not open', async () => {
		getPullRequestMock.mockResolvedValueOnce({ ...prResponse, state: 'closed', draft: false })
	...
	it('skips when PR is in draft', async () => {
		getPullRequestMock.mockResolvedValueOnce({ ...prResponse, draft: true })
	...
	it('skips when card is moved to changes requested list', async () => {
		...
		await updateCardMembers({ ...conf, trelloListIdPrChangesRequested: '1' }, ['card'])
	...
	it('skips when card is moved to approved list', async () => {
		...
		await updateCardMembers({ ...conf, trelloListIdPrApproved: '1' }, ['card'])
	...
	it('skips when turned off', async () => {
		await updateCardMembers({ ...conf, trelloSwitchMembersInReview: false }, ['card'])
